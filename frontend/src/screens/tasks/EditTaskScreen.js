...
const EditTaskScreen = ({ route, navigation }) => {
  ...
  const [estimatedTime, setEstimatedTime] = useState(String(task.estimatedTime || 60));
          ...
          <View style={styles.statusContainer}>
            {statuses.map((item) => (
              <TouchableOpacity
                ...
                style={[
                  styles.statusOption,
                  status === item.key && { backgroundColor: item.color },
                ]}
              ...
              >
                <Text
                  style={[
                    styles.statusLabel,
                    status === item.key && styles.statusLabelActive,
                  ]}
                >
                  {item.label}
                </Text>
